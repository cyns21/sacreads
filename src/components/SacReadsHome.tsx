...
import type { BookRecommendation, ClientBook } from "@/types/book";

type SacReadsHomeProps = {
  initialBooks: BookRecommendation[];
};

export function SacReadsHome({ initialBooks }: SacReadsHomeProps) {
  const [previewBooks, setPreviewBooks] = useState<ClientBook[]>(initialBooks.slice(0, 3));
  const [savedCount, setSavedCount] = useState(0);

  function handleRecommendationsLoaded(recommendedBooks: ClientBook[]) {
    setPreviewBooks(recommendedBooks.slice(0, 3));
  }
            ...
            <div className="rounded-lg border border-[#d8ccb9] bg-[#fffaf1] p-5 shadow-sm">
              <div className="mb-5 flex items-center justify-between gap-4 border-b border-[#e4dacb] pb-4">
                <div>
                  ...
                  <p className="mt-1 text-xs font-medium text-[#6a6257]">Latest search preview</p>
                </div>
                <span className="rounded-md bg-[#cbd8bc] px-3 py-2 text-xs font-bold text-[#214d45]">
                  {previewBooks.length} matches
                </span>
              </div>

              <div className="grid gap-3">
                {previewBooks.map((book) => (
                  <div className="grid grid-cols-[56px_1fr] gap-3 rounded-md bg-white p-3" key={book.id}>
                    {book.coverImageUrl ? (
                      <Image
                        alt=""
                      ...
                      <div className="h-20 rounded-md bg-[#cbd8bc]" />
                    )}
